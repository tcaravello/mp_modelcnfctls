...
if indic_models == 1
    for i_model = 1:n_models
        if i_model <= 2
            ...
            plot(date(fcst_date-fcst_lag):0.25:date(fcst_date+fcst_hor-1),pi_cnfctl_models(:,i_model),'-.','Color',settings.colors.models(i_model-2,:),'LineWidth',3)
            hold on
        end
    end
end
...
if indic_models == 1
    for i_model = 1:n_models
        if i_model <= 2
            ...
            plot(date(fcst_date-fcst_lag):0.25:date(fcst_date+fcst_hor-1),y_cnfctl_models(:,i_model),'-.','Color',settings.colors.models(i_model-2,:),'LineWidth',3)
            hold on
        end
    end
end
...
if indic_models == 1
    for i_model = 1:n_models
        if i_model <= 2
            ...
            plot(date(fcst_date-fcst_lag):0.25:date(fcst_date+fcst_hor-1),i_cnfctl_models(:,i_model),'-.','Color',settings.colors.models(i_model-2,:),'LineWidth',3)
            hold on
        end
    end
end
...
% ylabel('\% Deviation','interpreter','latex','FontSize',20)
if indic_models == 1
    legend({'History','Counterfct''l','','','RANK','HANK','B-RANK','B-HANK'},...
        'Location','Southwest','Orientation','horizontal','fontsize',18,'interpreter','latex','NumColumns',2)
else
