...
pi_cnfctl_models_lb = quantile(pi_cnfctl_models,0.16,3);
y_cnfctl_models_lb  = quantile(y_cnfctl_models,0.16,3);
i_cnfctl_models_lb  = quantile(i_cnfctl_models,0.16,3);
r_cnfctl_models_lb  = quantile(r_cnfctl_models,0.16,3);
m_cnfctl_models_lb  = quantile(m_cnfctl_models,0.16,3);

pi_cnfctl_models_med = quantile(pi_cnfctl_models,0.5,3);
y_cnfctl_models_med  = quantile(y_cnfctl_models,0.5,3);
i_cnfctl_models_med  = quantile(i_cnfctl_models,0.5,3);
r_cnfctl_models_med  = quantile(r_cnfctl_models,0.5,3);
m_cnfctl_models_med  = quantile(m_cnfctl_models,0.5,3);

pi_cnfctl_models_ub = quantile(pi_cnfctl_models,0.84,3);
y_cnfctl_models_ub  = quantile(y_cnfctl_models,0.84,3);
i_cnfctl_models_ub  = quantile(i_cnfctl_models,0.84,3);
r_cnfctl_models_ub  = quantile(r_cnfctl_models,0.84,3);
m_cnfctl_models_ub  = quantile(m_cnfctl_models,0.84,3);
...
settings.colors.lmodels = [0.25 * [196/255 174/255 120/255] + 0.75 * [1 1 1]; ... % beige
                            0.25 * [204/255 0/255 0/255] + 0.75 * [1 1 1]; ... % red
                            0.25 * [102/255 178/255 255/255] + 0.75 * [1 1 1]]; % blue
...
if indic_emp == 0 && indic_models == 1
    pi_cnfctl_models_lb  = [repmat(pi_history(fcst_date-fcst_lag:fcst_date),1,n_models);pi_cnfctl_models_lb];
    pi_cnfctl_models_med = [repmat(pi_history(fcst_date-fcst_lag:fcst_date),1,n_models);pi_cnfctl_models_med];
    pi_cnfctl_models_ub  = [repmat(pi_history(fcst_date-fcst_lag:fcst_date),1,n_models);pi_cnfctl_models_ub];
end
...
if indic_emp == 0 && indic_models == 1
    pi_cnfctl_models_lb  = pi_cnfctl_models_lb + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,pi_pos);
    pi_cnfctl_models_med = pi_cnfctl_models_med + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,pi_pos);
    pi_cnfctl_models_ub  = pi_cnfctl_models_ub + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,pi_pos);
end
...
if indic_emp == 0 && indic_models == 1
    y_cnfctl_models_lb  = [repmat(y_history(fcst_date-fcst_lag:fcst_date),1,n_models);y_cnfctl_models_lb];
    y_cnfctl_models_med = [repmat(y_history(fcst_date-fcst_lag:fcst_date),1,n_models);y_cnfctl_models_med];
    y_cnfctl_models_ub  = [repmat(y_history(fcst_date-fcst_lag:fcst_date),1,n_models);y_cnfctl_models_ub];
end
...
if indic_emp == 0 && indic_models == 1
    y_cnfctl_models_lb  = y_cnfctl_models_lb + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,y_pos);
    y_cnfctl_models_med = y_cnfctl_models_med + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,y_pos);
    y_cnfctl_models_ub  = y_cnfctl_models_ub + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,y_pos);
end
...
if indic_emp == 0 && indic_models == 1
    i_cnfctl_models_lb  = [repmat(i_history(fcst_date-fcst_lag:fcst_date),1,n_models);i_cnfctl_models_lb];
    i_cnfctl_models_med = [repmat(i_history(fcst_date-fcst_lag:fcst_date),1,n_models);i_cnfctl_models_med];
    i_cnfctl_models_ub  = [repmat(i_history(fcst_date-fcst_lag:fcst_date),1,n_models);i_cnfctl_models_ub];
end
...
if indic_emp == 0 && indic_models == 1
    i_cnfctl_models_lb  = i_cnfctl_models_lb + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos);
    i_cnfctl_models_med = i_cnfctl_models_med + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos);
    i_cnfctl_models_ub  = i_cnfctl_models_ub + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos);
end
...
if indic_emp == 0 && indic_models == 1
    r_cnfctl_models_lb  = [repmat(r_history(fcst_date-fcst_lag:fcst_date),1,n_models);r_cnfctl_models_lb];
    r_cnfctl_models_med = [repmat(r_history(fcst_date-fcst_lag:fcst_date),1,n_models);r_cnfctl_models_med];
    r_cnfctl_models_ub  = [repmat(r_history(fcst_date-fcst_lag:fcst_date),1,n_models);r_cnfctl_models_ub];
end
...
r_cnfctl_ub     = r_cnfctl_ub + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos) ...
                    - [det_X_ext(fcst_date-fcst_lag+1:fcst_date+fcst_hor,:); det_X_ext(end,:)] * det_coeff(:,pi_pos);
if indic_emp == 0 && indic_models == 1
    r_cnfctl_models_lb  = r_cnfctl_models_lb + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos) ...
                    - [det_X_ext(fcst_date-fcst_lag+1:fcst_date+fcst_hor,:); det_X_ext(end,:)] * det_coeff(:,pi_pos);
    r_cnfctl_models_med = r_cnfctl_models_med + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos) ...
                    - [det_X_ext(fcst_date-fcst_lag+1:fcst_date+fcst_hor,:); det_X_ext(end,:)] * det_coeff(:,pi_pos);
    r_cnfctl_models_ub  = r_cnfctl_models_ub + det_X_ext(fcst_date-fcst_lag:fcst_date+fcst_hor,:) * det_coeff(:,i_pos) ...
                    - [det_X_ext(fcst_date-fcst_lag+1:fcst_date+fcst_hor,:); det_X_ext(end,:)] * det_coeff(:,pi_pos);
end
...
hold on
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    jbfill(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),(pi_cnfctl_models_lb(:,indx_model))',(pi_cnfctl_models_ub(:,indx_model))',...
        settings.colors.lmodels(indx_model,:),settings.colors.lmodels(indx_model,:),0,0.5);
    hold on
end
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    plot(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),pi_cnfctl_models_med(:,indx_model),...
        '-','Color',settings.colors.models(indx_model,:),'LineWidth',4)
    hold on
end
...
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    jbfill(0,0,0,settings.colors.lmodels(indx_model,:),settings.colors.lmodels(indx_model,:),0,1);
    hold on
end
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    jbfill(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),(y_cnfctl_models_lb(:,indx_model))',(y_cnfctl_models_ub(:,indx_model))',...
        settings.colors.lmodels(indx_model,:),settings.colors.lmodels(indx_model,:),0,0.5);
    hold on
end
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    plot(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),y_cnfctl_models_med(:,indx_model),...
        '-','Color',settings.colors.models(indx_model,:),'LineWidth',4)
    hold on
end
...
if i_figure == 1
    mod_str = 'HANK Cnfct''l';
else
     mod_str = 'B-RANK Cnfct''l';
end

legend({'History','Forecast','RANK Cnfct''l',mod_str},'Location','Southeast','fontsize',18,...
    'NumColumns',2','Orientation','horizontal','interpreter','latex')
...
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    jbfill(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),(i_cnfctl_models_lb(:,indx_model))',(i_cnfctl_models_ub(:,indx_model))',...
        settings.colors.lmodels(indx_model,:),settings.colors.lmodels(indx_model,:),0,0.5);
    hold on
end
for i_model = 1:2
    indx_model = model_indic(i_figure,i_model);
    plot(date_ext(fcst_date-fcst_lag):0.25:date_ext(fcst_date+fcst_hor),i_cnfctl_models_med(:,indx_model),...
        '-','Color',settings.colors.models(indx_model,:),'LineWidth',4)
    hold on
end
...
if save_fig == 1
print(['scenario_2021_optpol_' num2str(i_figure)],'-dpng');
end
end
