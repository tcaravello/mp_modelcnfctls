...
% reduce variance on the interest rate for AD

%Psi_i_var_aux = Psi_i_var;

% Psi_i_var(1:3*T_use,1:3*T_use) = Psi_i_var(1:3*T_use,1:3*T_use)/scale_ad;
% Psi_i_var(1:2*T_use,1:2*T_use) = Psi_i_var_aux(1:2*T_use,1:2*T_use);
Psi_i_var(2*T_use+1:3*T_use,:) = Psi_i_var(2*T_use+1:3*T_use,:)/(scale_ad);
Psi_i_var(:,2*T_use+1:3*T_use) = Psi_i_var(:,2*T_use+1:3*T_use)/(scale_ad);
